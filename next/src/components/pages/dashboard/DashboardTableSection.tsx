  ...
  const handleAddCategorySubmit = async (formData: CategoryFormData) => {
    try {
      ...
      if (statusCode === 201) {
        ...
        showNotification("เกิดข้อผิดพลาดในการเพิ่มหมวดหมู่", "error");
      }
    } catch (error) {
  ...
  const handleEditCategorySubmit = async (formData: CategoryFormData) => {
    ...
    try {
      ...
      if (statusCode === 200) {
        ...
        showNotification("แก้ไขหมวดหมู่สำเร็จ!", "success");
        if (onRefresh) {
          onRefresh();
        }
      } else if (statusCode === 400) {
        ...
        showNotification("เกิดข้อผิดพลาดในการแก้ไขหมวดหมู่", "error");
      }
    } catch (error) {
          ...
          >
            ...
            {categoryToDelete.indicators &&
              categoryToDelete.indicators.length > 0 && (
                ...
                >
                  ⚠️ คำเตือน: หมวดหมู่นี้มี {categoryToDelete.indicators.length}{" "}
                  ตัวชี้วัด
                </Typography>
              )}
          </Box>
