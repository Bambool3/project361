...
export default function EmployeeTableSection({
  ...
  useEffect(() => {
    const fetchRoles = async () => {
      try {
        ...
        showNotification("ไม่สามารถโหลดข้อมูลตำแหน่งได้", "error");
      } finally {
  ...
  const handleAddEmployeeSubmit = async (formData: EmployeeFormData) => {
    try {
      ...
      if (statusCode === 201) {
        ...
        showNotification("บุคลากรคนนี้มีอยู่ในระบบแล้ว", "warning");
        ...
        showNotification("เกิดข้อผิดพลาดในการเพิ่มบุคลากร", "error");
      }
    } catch (error) {
  ...
  const handleEditEmployeeSubmit = async (formData: EmployeeFormData) => {
    ...
    try {
      ...
      if (statusCode === 200) {
        ...
        showNotification("แก้ไขข้อมูลบุคลากรสำเร็จ!", "success");
        if (onRefresh) {
          onRefresh();
        }
      } else if (statusCode === 400) {
        ...
        showNotification("ไม่พบบุคลากรที่ต้องการแก้ไข", "error");
      } else if (statusCode === 409) {
        showNotification(
        ...
        showNotification("เกิดข้อผิดพลาดในการแก้ไขบุคลากร", "error");
      }
    } catch (error) {
  ...
  const handleExportExcel = () => {
    // TODO
    showNotification("ฟีเจอร์ส่งออก Excel จะพัฒนาในอนาคต", "info");
  };

  const handleImportExcel = () => {
    // TODO
    showNotification("ฟีเจอร์นำเข้า Excel จะพัฒนาในอนาคต", "info");
  };
  ...
  const handleDeleteEmployeeConfirm = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        showNotification("ลบบุคลากรสำเร็จ!", "success");
        if (onRefresh) {
          onRefresh();
        }
        ...
        showNotification(
          data.error || "ไม่สามารถลบบุคลากรที่มีตัวชี้วัดที่เกี่ยวข้องได้",
          "warning"
        );
      } else if (response.status === 404) {
        showNotification("ไม่พบบุคลากรที่ต้องการลบ", "error");
      } else {
        showNotification("เกิดข้อผิดพลาดในการลบบุคลากร", "error");
      }
    } catch (error) {
          ...
          >
            ...
            >
              ...
              <FormControl size="small" sx={{ minWidth: 150 }}>
                <InputLabel id="department-select-label">หน่วยงาน</InputLabel>
                <Select
                  ...
                  paginatedEmployees.map((employee, index) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        >
                          {employee.job_titles.length > 0 ? (
                            employee.job_titles.map((jobTitle, index) => (
                              <Chip
                                key={jobTitle.id}
                                label={jobTitle.name}
                                size="small"
