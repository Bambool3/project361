...
export default function FrequencyTableSection({
  ...
  const [frequencyToDelete, setFrequencyToDelete] = useState<Frequency | null>(
    null
  );
  ...
  const handleAddFrequencySubmit = async (formData: FrequencyFormData) => {
    try {
      ...
      if (response.status === 201) {
        ...
        showNotification("เพิ่มความถี่สำเร็จ!", "success");
        if (onRefresh) {
          onRefresh();
        }
        ...
        showNotification("ความถี่นี้มีอยู่ในระบบแล้ว", "warning");
      } else {
        showNotification("เกิดข้อผิดพลาดในการเพิ่มความถี่", "error");
      }
    } catch (error) {
  ...
  const handleEditFrequency = async (frequencyId: string) => {
    try {
      ...
      showNotification("เกิดข้อผิดพลาดในการโหลดข้อมูลความถี่", "error");
    }
  };
  ...
  const handleEditFrequencySubmit = async (formData: FrequencyFormData) => {
    ...
    try {
      ...
      if (response.status === 200) {
        ...
        showNotification("แก้ไขความถี่สำเร็จ!", "success");
        if (onRefresh) {
          onRefresh();
        }
      } else if (response.status === 400) {
        ...
        showNotification("ไม่พบความถี่ที่ต้องการแก้ไข", "error");
      } else {
        showNotification("เกิดข้อผิดพลาดในการแก้ไขความถี่", "error");
      }
    } catch (error) {
  ...
  const handleDeleteFrequency = (frequencyId: string) => {
    const frequency = frequencies.find((f) => f.frequency_id === frequencyId);
    if (frequency) {
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      ...
      if (response.status === 200) {
        showNotification("ลบความถี่สำเร็จ!", "success");
        if (onRefresh) {
          onRefresh();
        }
      } else if (response.status === 400) {
        ...
        showNotification(
          data.error || "ไม่สามารถลบความถี่ที่มีตัวชี้วัดที่เกี่ยวข้องได้",
          "warning"
        );
      } else if (response.status === 404) {
        showNotification("ไม่พบความถี่ที่ต้องการลบ", "error");
      } else {
        showNotification("เกิดข้อผิดพลาดในการลบความถี่", "error");
      }
    } catch (error) {
            ...
            <Table>
              ...
              <TableBody>
                {paginatedFrequencies.length === 0 ? (
                  ...
                  paginatedFrequencies.map((frequency, index) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        >
                          ...
                          >
                            {formatDateRange(frequency.periods)}
                          </Typography>
                        </TableCell>
                        ...
                        >
                          ...
                          >
                            {frequency.indicatorCount || 0} ตัวชี้วัด
                          </Typography>
                        </TableCell>
                        ...
                        >
                          ...
                          >
                            ...
                            >
                              <IconButton
                                onClick={() =>
                                  handleEditFrequency(frequency.frequency_id)
                                }
                                size="small"
                                sx={{
                                  color: "#64748b",
                            ...
                            >
                              <IconButton
                                onClick={() =>
                                  handleDeleteFrequency(frequency.frequency_id)
                                }
                                size="small"
                                sx={{
                                  color: "#64748b",
