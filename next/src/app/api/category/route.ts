...
export async function GET(request: Request) {
    try {
        ...
        if (userId && filterBy === "responsible") {
            categories =
                await CategoryServerService.getCategoriesByResponsibleUser(
                    userId
                );
        } else if (userId && filterBy === "jobtitle") {
            categories =
                await CategoryServerService.getCategoriesByResponsibleJobTitle(
                    userId
                );
        } else if (userId && filterBy === "created") {
            categories = await CategoryServerService.getCategoriesByUser(
                userId
            );
        } else if (userId) {
            categories = await CategoryServerService.getCategoriesByUser(
            ...
            categories = await CategoryServerService.getCategories();
        }
