...
export class CategoryServerService {
  ...
  static async getCategoriesByUser(userId: string): Promise<Category[]> {
    try {
      ...
      console.error("Error fetching categories for user from database:", error);
      throw new Error("Failed to fetch categories for user from database");
    }
  }
  ...
  static async getCategoryByName(name: string): Promise<Category | null> {
    try {
      ...
      console.error("Error fetching category by name from database:", error);
